...
def parse_args():
    parser = argparse.ArgumentParser(description="Generate Figure 3D XML files for all 7TF self-competition combinations.")
    parser.add_argument("--data-dir", type=Path, default=Path("~/Desktop/SB_final/ScienceDirect_files_26May2026_04-41-20.049").expanduser(),
                        help="Directory containing mmc2/mmc3/mmc4 workbooks.")
    parser.add_argument("--mmc2", type=Path, default=None)
    parser.add_argument("--mmc3", type=Path, default=None)
    ...
    parser.add_argument("--init-loop", type=int, default=100000)
...
def make_xml(tfs_xml, tf_data, gene_lines, rate_line, theta, seed, self_comp=True, num_threads=1, init_loop=100000):
    sc = "true" if self_comp else "false"
    return f"""<?xml version="1.0" encoding="utf-8"?>
<Root>
  <annealer_input init_T="100000" lambda="0.0001" init_loop="{init_loop}"/>
...
def main():
    args = parse_args()
    mmc2 = args.mmc2 or args.data_dir / "1-s2.0-S2589004223028249-mmc2.xlsx"
    mmc3 = args.mmc3 or args.data_dir / "1-s2.0-S2589004223028249-mmc3.xlsx"
    mmc4 = args.mmc4 or args.data_dir / "1-s2.0-S2589004223028249-mmc4.xlsx"
    xml_dir = args.xml_dir or args.fits_dir / "fig3de_xmls"
    ...
    pwms        = load_pwms(mmc3)
    params      = load_params(mmc2)
    gene_lines, rate_line = load_single_hit_data(mmc4)
    ...
    for n_tfs in range(1, 8):
        for combo in combinations(TFS, n_tfs):
            ...
            for m_num in range(1, 9):
                ...
                xml      = make_xml(tfs_xml, tf_data, gene_lines, rate_line, theta, m_num,
                                    self_comp=True, num_threads=args.num_threads, init_loop=args.init_loop)
                fname    = xml_dir / f"fig3de_n{n_tfs}_m{m_num}_{combo_name}.xml"
